...
export async function openStateDrawer({ ctx, state, metadata, rebuild, reset, retryAnalysis, analyzeMissing, cancelAnalysis, reanalyzeChat, setNpcStatusAndRebuild }) {
    if (!state) return;
    ...
    const bindNpcActions = () => {
        root.querySelectorAll('[data-npc-status]').forEach(button => button.addEventListener('click', async () => {
            button.disabled = true;
            const changed = await setNpcStatusAndRebuild(button.dataset.npcId, button.dataset.npcStatus);
